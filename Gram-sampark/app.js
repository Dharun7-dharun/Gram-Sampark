    ...
    if (user) {
        ...
        if (userRole === 'admin') {
            ...
            setupTabs([
                ...
                { id: 'panel-admin-certificates', title: 'Certificate Approvals' },
            ...
            setupTabs([
                ...
                { id: 'panel-upload-certificate', title: 'Upload Certificates' }
            ]);
            userSetup();
            await fetchAssignedVillages(user);
            setupPatientListener();
...
// Certificate Upload & Approval Logic
const certUploadForm = document.getElementById('certificate-upload-form');
const certTypeSelect = document.getElementById('certificate-type');
const certUploadInput = document.getElementById('certificate-upload');
const certUploadName = document.getElementById('file-upload-name');
const certUploadMsg = document.getElementById('cert-upload-msg');
...
if (certUploadInput) {
    certUploadInput.addEventListener('change', (e) => {
        if (e.target.files.length > 0) {
            certUploadName.textContent = e.target.files[0].name;
            certUploadName.style.color = 'var(--accent)';
        } else {
            certUploadName.textContent = 'Click to browse or drag and drop';
            certUploadName.style.color = 'var(--text)';
        }
    });
}
...
if (certUploadForm) {
    certUploadForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const residentName = document.getElementById('cert-resident-name').value.trim();
        const type = certTypeSelect.value;
        const file = certUploadInput.files[0];

        if (!file) {
            certUploadMsg.textContent = 'Please select a file.';
            certUploadMsg.className = 'error';
            return;
        }

        const submitBtn = document.getElementById('submit-certificate-btn');
        submitBtn.disabled = true;
        submitBtn.textContent = 'Uploading...';

        try {
            // Upload to Firebase Storage
            const storageRef = ref(storage, `certificates/${currentUser.uid}_${Date.now()}_${file.name}`);
            ...
            certUploadMsg.textContent = 'Certificate uploaded successfully and sent for approval.';
            certUploadMsg.className = 'success';
            certUploadForm.reset();
            certUploadName.textContent = 'Click to browse or drag and drop';
            certUploadName.style.color = 'var(--text)';
            
            setTimeout(() => { certUploadMsg.textContent = ''; }, 4000);
        } catch (error) {
            certUploadMsg.textContent = `Upload failed: ${error.message}`;
            certUploadMsg.className = 'error';
        } finally {
            submitBtn.disabled = false;
            submitBtn.textContent = 'Send for Admin Approval';
        }
    });
}
...
window.setupCertificatesListener = function() {
    if (certificatesUnsubscribe) certificatesUnsubscribe();
    
    let q;
    if (userRole === 'admin') {
        q = query(collection(db, 'certificate_requests'), orderBy('created_at', 'desc'));
    } else {
        q = query(collection(db, 'certificate_requests'), where('user_email', '==', currentUser.email), orderBy('created_at', 'desc'));
    }

    certificatesUnsubscribe = onSnapshot(q, (snapshot) => {
        if (myCertificatesList) myCertificatesList.innerHTML = '';
        if (adminCertificatesList) adminCertificatesList.innerHTML = '';
        
        let pendingCount = 0;
        
        snapshot.forEach(docSnap => {
            const data = docSnap.data();
            const date = data.created_at ? new Date(data.created_at.toDate()).toLocaleDateString() : 'N/A';
            const isApproved = data.status === 'approved';
            const isRejected = data.status === 'rejected';
            
            const badgeClass = isApproved ? 'badge-success' : (isRejected ? 'error' : 'badge-warning');
            ...
            if (userRole === 'admin') {
                ...
                div.innerHTML = `
                    <div class="cert-header">
                        <div class="cert-info">
                            <h3>${escapeHTML(data.resident_name)}</h3>
                            <p>${escapeHTML(data.user_email)}</p>
                            <p class="secondary-text" style="font-size: 0.8rem;">Submitted: ${date}</p>
                        </div>
                        <div class="cert-type">${escapeHTML(data.type)}</div>
                    </div>
                    
                    <a href="${escapeHTML(data.file_url)}" target="_blank" class="cert-view-btn">
                    ...
                    <div class="cert-actions" style="margin-top: auto;">
                        ${data.status === 'pending' ? `
                            <button class="icon-btn" style="flex: 1;" onclick="updateCertStatus('${docSnap.id}', 'approved')">Approve</button>
                            <button class="icon-btn delete" style="flex: 1;" onclick="updateCertStatus('${docSnap.id}', 'rejected')">Reject</button>
                        ` : `
                            <div style="text-align: center; width: 100%;">
                                <span class="badge ${badgeClass}" style="width: 100%; justify-content: center;">${data.status.toUpperCase()}</span>
                            </div>
                        `}
                    </div>
                `;
                if (adminCertificatesList) adminCertificatesList.appendChild(div);
            } else {
                div.innerHTML = `
                    <div class="cert-header">
                        <div class="cert-info">
                    ...
                    <a href="${escapeHTML(data.file_url)}" target="_blank" class="cert-view-btn" style="margin-top: auto;">
                        <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                            <polyline points="14 2 14 8 20 8"></polyline>
                        </svg>
                ...
                `;
                if (myCertificatesList) myCertificatesList.appendChild(div);
            }
        });
        
        if (adminCertificatesList && adminCertificatesList.innerHTML === '') {
            adminCertificatesList.innerHTML = '<div class="empty-state-card">No certificate requests found.</div>';
        }
        if (myCertificatesList && myCertificatesList.innerHTML === '') {
            myCertificatesList.innerHTML = '<div class="empty-state-card">You have not submitted any certificates.</div>';
        }
    });
}

window.updateCertStatus = async function(id, status) {
    try {
        await setDoc(doc(db, 'certificate_requests', id), { status: status, updated_at: serverTimestamp() }, { merge: true });
    } catch (e) {
        alert(e.message);
    }
};
